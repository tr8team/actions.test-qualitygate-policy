...
type MinPercentageTestPolicyConfig = z.infer<typeof minPercentageTestPolicy>;

class MinPercentageTestPolicy extends BasePolicy implements Policy {
  readonly #config: MinPercentageTestPolicyConfig;

  constructor(
    name: string,
    target: string,
    config: MinPercentageTestPolicyConfig
  ) {
    super(name, target);
    this.#config = config;
  }
  ...
  evaluate(current: IntermediateEntry): Promise<IntermediateEntry> {
    return this.filteredEvaluate(current, (item) => {
      if (item.data.type === "test-result") {
        ...
        const percentage =
          total === 0 ? 100 : (data[this.#config.metric] / total) * 100;
...
export type { MinPercentageTestPolicyConfig };
